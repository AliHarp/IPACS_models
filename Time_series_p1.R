...
SEED <-1
nruns <- 200 #was 200
sim_length <- 40 #was 730
warmup <-1000 #was 1000
...
n_patients = 170 #number of patients system take
#initialise n_slots
...
#write.csv(Final, file = "fixed capacity_trialling.csv")
...
ggplot(data=ts_output_quants, aes(x=Date, y=q5_res_used, group = 1)) +
  theme_bw()+
  theme(panel.border = element_rect(fill = NA, color = "grey50", size = 0.5, linetype="solid"))+
